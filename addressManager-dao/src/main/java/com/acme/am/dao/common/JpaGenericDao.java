package com.acme.am.dao.common;
...
public abstract class JpaGenericDao<E extends Serializable> implements GenericDao<E> {
	...
	/**
	 * Führt das JPA-QL-Statement <tt>stmt</tt> mit den Parametern
	 * <tt>parameters</tt> aus.
	 * 
	 * @param stmt
	...
	/**
	 * Führt das NamedQuery <tt>queryName</tt> mit den Parametern
	 * <tt>parameters</tt> aus und liefert ab der Zeile <tt>firstResult</tt> aber
	 ...
	 * @return eine Liste der Objekte.
	 */
	public List<E> findByNamedQuery(final String queryName, final Map<String, Object> parameters, final int firstResult,
	...
	private void checkMaxResult(final int maxResults) {
		if (maxResults < 1) {
			final String msg = MessageFormat.format("maxResults darf nicht kleiner als 1 sein [{0}]",
	 ...
	 * 
	 * @param params
	 *            must be an even number of alternating arguments, where the first
	 *            argument is a string (key) and the second an arbitrary object
	 *            (value).
	 * 
